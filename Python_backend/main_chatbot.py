...
async def generate_response(prompt, tavily_client, gemini_model, memory, emergency_vector_store, legal_vector_store, mental_vector_store, self_defence_vector_store):
    ...
    if not is_women_safety_related(prompt):
        print("Not women safety related")
        history = memory.load_memory_variables({})
        history_context = "\n".join([f"{m.type}: {m.content}" for m in history.get("history", [])])
        new_prompt = f"Conversation History:\n{history_context}\n\prompt: {prompt}\n\n"
        response = await get_gemini_response(new_prompt, "You are a helpful assistant that provides information about women safety.")
        ...
        if Agent == "emergency services":        
            ...
            context = await retrieve_context(prompt, mental_vector_store)  
            ...
            context = await retrieve_context(prompt, self_defence_vector_store)  
    ...
    return await get_gemini_response(full_prompt, system_prompt) , Agent


def is_women_safety_related(prompt):
    # Creating SLM client
    SLM = InferenceClient(
